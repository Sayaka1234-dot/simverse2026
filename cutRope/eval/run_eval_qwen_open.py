...
DEFAULT_BASE_URL = os.getenv("QWEN_OPEN_BASE_URL", os.getenv("SILICONFLOW_BASE_URL", "https://api.siliconflow.cn/v1"))
DEFAULT_API_KEY = os.getenv("QWEN_OPEN_API_KEY", os.getenv("SILICONFLOW_API_KEY", ""))
DEFAULT_TIMEOUT_SECONDS = float(os.getenv("QWEN_OPEN_TIMEOUT_SECONDS", "600"))
DEFAULT_MAX_TOKENS = int(os.getenv("QWEN_OPEN_MAX_TOKENS", "20000"))
DEFAULT_TRUST_ENV = os.getenv("QWEN_OPEN_TRUST_ENV", "false").strip().lower() in {
    "1",
    "true",
    "yes",
    "on",
...
def build_qwen_open_argument_parser():
    parser = build_argument_parser(
        "Run Cut the Rope video-to-command evaluation with open-source Qwen endpoints.",
...
def main() -> int:
    parser = build_qwen_open_argument_parser()
    args = parser.parse_args()
    if not args.api_key:
        raise SystemExit("Missing API key. Set QWEN_OPEN_API_KEY/SILICONFLOW_API_KEY or pass --api-key.")
