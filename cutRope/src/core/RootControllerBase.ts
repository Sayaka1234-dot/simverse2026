...
class RootControllerBase extends ViewController {
    ...
    private getPointerTargetElement(): HTMLElement | null {
    ...
    private shouldHandleGameplayPointer(event: Event): boolean {
        const path = typeof event.composedPath === "function" ? event.composedPath() : [];
        const pathTarget = path.find((entry): entry is HTMLElement => entry instanceof HTMLElement);
        const target =
            pathTarget ?? (event.target instanceof HTMLElement ? event.target : this.getPointerTargetElement());
        ...
        if (target.closest(blockedSelector) != null) {
            return false;
        }
    ...
    activateMouseEvents(): void {
        const element = this.getPointerTargetElement();
        const positionElement = Canvas.element;
        if (!element || !positionElement) {
            return;
        }

        if (this.pointerCapture && this.pointerCapture.el !== element) {
        ...
        if (!this.pointerCapture) {
            this.pointerCapture = new PointerCapture({
                element,
                positionElement,
                shouldHandleEvent: this.shouldHandleGameplayPointer.bind(this),
            ...
            this.pointerCapture.positionElement = positionElement;
        }
