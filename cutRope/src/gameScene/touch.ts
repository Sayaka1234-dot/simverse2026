...
class GameSceneTouch extends GameSceneUpdate {
    ...
    touchDown(x: number, y: number, touchIndex: number): boolean {
        ...
        if (this.miceManager?.handleClick(cameraAdjustedX, cameraAdjustedY)) {
            manualCommandRecorder.recordTapMouse(this);
            return true;
        }

        if (this.candyBubble) {
            const bubbleIndex = this.bubbles.indexOf(this.candyBubble);
            if (this.handleBubbleTouch(this.star, x, y)) {
                manualCommandRecorder.recordPopBubble(bubbleIndex, this);
                return true;
            }
        }
        ...
        if (this.twoParts !== GameSceneConstants.PartsType.NONE) {
            if (this.candyBubbleL) {
                const bubbleIndex = this.bubbles.indexOf(this.candyBubbleL);
                if (this.handleBubbleTouch(this.starL, x, y)) {
                    if (bubbleIndex >= 0) {
                        manualCommandRecorder.recordPopBubble(bubbleIndex, this);
                    } else {
                        manualCommandRecorder.recordPopBubbleLeft(this);
                    }
                    return true;
                }
            }
            if (this.candyBubbleR) {
                const bubbleIndex = this.bubbles.indexOf(this.candyBubbleR);
                if (this.handleBubbleTouch(this.starR, x, y)) {
                    if (bubbleIndex >= 0) {
                        manualCommandRecorder.recordPopBubble(bubbleIndex, this);
                    } else {
                        manualCommandRecorder.recordPopBubbleRight(this);
                    }
                    return true;
                }
            }
        ...
        if (this.lightbulbs.length > 0) {
            for (const [bulbIndex, bulb] of this.lightbulbs.entries()) {
                if (!bulb?.capturingBubble) {
                    continue;
                }
                if (this.handleLightBulbBubbleTouch(bulb, x, y)) {
                    manualCommandRecorder.recordPopLightBulbBubble(bulbIndex, this);
                    return true;
                }
            }
        }
        ...
        // handle pump touches
        for (const [pumpIndex, pump] of this.pumps.entries()) {
            if (!pump) {
                continue;
            }
            if (pump.pointInObject(cameraAdjustedX, cameraAdjustedY)) {
                ...
                manualCommandRecorder.recordActivatePump(pumpIndex, this);
                return true;
            }
        }
        ...
        if (!this.noCandy) {
            for (const grab of this.bungees) {
                ...
                if (
                    ...
                    manualCommandRecorder.recordFireGun(this.bungees.indexOf(grab), this);
                    return true;
                }
            }
        }

        for (const [tubeIndex, tube] of this.tubes.entries()) {
            if (tube?.onTouchDown(cameraAdjustedX, cameraAdjustedY)) {
                manualCommandRecorder.recordToggleSteamTube(tubeIndex, this);
                return true;
            }
        }

        for (const [lanternIndex, lantern] of this.lanterns.entries()) {
            if (lantern?.onTouchDown(cameraAdjustedX, cameraAdjustedY)) {
                ...
                manualCommandRecorder.recordReleaseLantern(lanternIndex, this);
                return true;
            }
        }
        ...
        for (const [ghostIndex, ghost] of this.ghosts.entries()) {
            if (ghost?.onTouchDown(cameraAdjustedX, cameraAdjustedY)) {
                manualCommandRecorder.recordTapGhost(ghostIndex, this);
                return true;
            }
        }
        ...
        if (this.clickToCut) {
            ...
            if (bungee?.highlighted && grab) {
                if (this.getNearestBungeeSegmentByConstraints(cutPos, grab)) {
                    const cutCount = this.cut(null, cutPos, cutPos, false);
                    if (cutCount > 0) {
                        manualCommandRecorder.recordCutRope(this.bungees.indexOf(grab), this);
                    }
                }
            }
        }
    ...
    touchUp(x: number, y: number, touchIndex: number): boolean {
        ...
        if (this.gravityButton && this.gravityTouchDown === touchIndex) {
            ...
            if (child?.isInTouchZone(x + this.camera.pos.x, y + this.camera.pos.y, true)) {
                ...
                manualCommandRecorder.recordToggleGravity(this);
            }
        ...
        for (const [grabIndex, grab] of this.bungees.entries()) {
            if (!grab) {
                continue;
            }
            ...
            if (grab.kickable && grab.rope) {
                ...
                if (
                    ...
                    manualCommandRecorder.recordKickRope(grabIndex, this);
                }
    ...
    touchMove(x: number, y: number, touchIndex: number): boolean {
        ...
        if (this.dragging[touchIndex]) {
            ...
            for (const fcc of currentCuts) {
                ...
                const uncutBefore = new Set<number>();
                for (const [grabIndex, grab] of this.bungees.entries()) {
                    if (grab?.rope && grab.rope.cut === Constants.UNDEFINED) {
                        uncutBefore.add(grabIndex);
                    }
                }

                const cutCount = this.cut(null, fcc.start, fcc.end, false);
                ropeCuts += cutCount;

                if (cutCount > 0) {
                    for (const [grabIndex, grab] of this.bungees.entries()) {
                        if (
                            uncutBefore.has(grabIndex) &&
                            grab?.rope &&
                            grab.rope.cut !== Constants.UNDEFINED
                        ) {
                            manualCommandRecorder.recordCutRope(grabIndex, this);
                        }
                    }
                }
            }
