...
function gameWon(scene: GameScene): void {
    ...
    // Allow the win animation (Om Nom chewing + candy shrink) to finish
    // before we transition to the results panel.
    const LEVEL_WON_DELAY_SECONDS = 0.6;
    ...
    scene.dd.callObject(scene, onLevelWonAppCallback, null, LEVEL_WON_DELAY_SECONDS);
    scene.dd.callObject(
        scene.gameController,
        scene.gameController.onLevelWon,
        null,
        LEVEL_WON_DELAY_SECONDS
    );
}
