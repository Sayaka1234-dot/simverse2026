...
import LevelFilenameOverlay from "@/ui/LevelFilenameOverlay";
...
declare global {
    interface Window {
        ctrAutomation?: {
            appReady: boolean;
            lastStartedLevel: number | null;
            startLevelCount: number;
            startLevel(levelNumber: number): Promise<number>;
        };
    }
}
...
class App {
    ...
    domReady(): void {
        ...
        GridOverlay.domReady();
        LevelFilenameOverlay.domReady();
    ...
    run(): void {
        ...
        PreLoader.run(() => {
            ...
            window.ctrAutomation = {
                appReady: true,
                lastStartedLevel: null,
                startLevelCount: 0,
                startLevel(levelNumber: number) {
                    const safeLevel = Math.max(1, Math.floor(levelNumber));
                    return new Promise<number>((resolve) => {
                        window.setTimeout(() => {
                            im.gameFlow.noMenuStartLevel(0, safeLevel - 1);
                            if (window.ctrAutomation) {
                                window.ctrAutomation.lastStartedLevel = safeLevel;
                                window.ctrAutomation.startLevelCount += 1;
                            }
                            resolve(safeLevel);
                        }, 0);
                    });
