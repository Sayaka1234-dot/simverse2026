...
class NetEdition {
    ...
    boxBorders: string[] = [];
    menuSoundIds: number[] = [];
    gameSoundIds: number[] = [ResourceId.SND_MONSTER_CHEWING, ResourceId.SND_WIN];
    ...
    getNormalizedBoxMetadata(): BoxMetadata[] {
        ...
        const primaryBox =
            normalizedMetadata.find((box) => box.boxType !== BoxType.HOLIDAY) ??
            normalizedMetadata[0];

        if (!primaryBox) {
            this._cachedNormalizedMetadata = [];
            return this._cachedNormalizedMetadata;
        }

        const unified: BoxMetadata = {
            ...primaryBox,
            id: "levels",
        ...
        this._cachedNormalizedMetadata = [unified];
        return this._cachedNormalizedMetadata;
    }
