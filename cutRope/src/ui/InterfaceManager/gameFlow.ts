...
import { startSnow, stopSnow } from "@/ui/InterfaceManager/constants";
import { fadeIn, fadeOut, show, hide, text, width } from "@/utils/domHelpers";
...
export default class GameFlow {
    ...
    _completeBox(): void {
        ...
        if (isGameComplete) {
            ...
            const targetPanelId = edition.disableBoxMenu ? PanelId.LEVELS : PanelId.BOXES;
    ...
    tapeBox(): void {
        ...
        manager.isShowingLevelResults = false;
    ...
    openBox(skip = false): void {
        ...
        this.manager.isShowingLevelResults = false;
        RootController.startLevel(BoxManager.currentBoxIndex + 1, BoxManager.currentLevelIndex);
        this.showGameUI();
    }

    /** Closes the box */
    closeBox(showResults = false): void {
        stopSnow();
        this.closeGameUI();

        if (!showResults && this.manager.isInLevelSelectMode) {
            this.manager.isShowingLevelResults = false;
            this.tapeBox();
            return;
        }

        this.manager.isShowingLevelResults = showResults;
        panelManager.showPanel(PanelId.LEVELS, true);
