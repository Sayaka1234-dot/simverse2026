...
const calculateTotalPages = (totalLevels: number): number => {
    return Math.max(1, Math.ceil(totalLevels / MAX_LEVELS_PER_PAGE));
};

const getLevelsPerPage = (totalLevels: number, pageIndex = 0): number => {
    const startIndex = pageIndex * MAX_LEVELS_PER_PAGE;
    return Math.min(MAX_LEVELS_PER_PAGE, Math.max(0, totalLevels - startIndex));
};
...
class LevelPanel extends Panel {
    ...
    onShow(): void {
        ...
        const levelPageIndicator = getElement("#levelPageIndicator");

        if (this.im?.isShowingLevelResults) {
            if (levelScore instanceof HTMLElement) {
                hide(levelScore);
            }
            if (levelPageIndicator instanceof HTMLElement) {
                hide(levelPageIndicator);
            }
        ...
        if (levelPageIndicator instanceof HTMLElement) {
            delay(levelPageIndicator, 200).then(() => {
                return fadeIn(levelPageIndicator, 700);
            });
        }
        if (levelBack instanceof HTMLElement) {
    ...
    private updateLevelOptions(): void {
        ...
        for (let i = 0; i < this.levelButtons.length; i++) {
            ...
            if (i < visibleCount && levelIndex < levelCount) {
                ...
                if (stars != null) {
                    ...
                    const levelData = edition.boxes[boxIndex]?.levels[levelIndex] as
                        | Record<string, unknown>
                        | undefined;
                    const levelId =
                        typeof levelData?.levelId === "string" ? levelData.levelId : null;
                    const levelLabel = levelId ?? String(levelIndex + 1);
                    const drawLabel = levelLabel.length > 3 ? Text.drawSmall : Text.drawBig;
                    append(
                        levelInfo,
                        drawLabel({ text: levelLabel, scaleToUI: true })
                    );
        ...
        Text.drawSmall({ text: pageText, imgSel: "#levelPageIndicator img", scaleToUI: true });
