...
class PreLoader {
    ...
    private checkGameLoadComplete(): void {
        if (!this.gameImagesLoadComplete || this.gameCompleteCallbacks.length === 0) {
            return;
        }

        const callbacks = this.gameCompleteCallbacks.splice(0);
    ...
    private collectImageResources(gameBaseUrl: string): {
        ...
        return { resources, menuResourceCount, gameResourceCount };
    }

    private loadImages(): { trackedResourceCount: number } {
        const gameBaseUrl = `${platform.imageBaseUrl}${resolution.CANVAS_WIDTH}/game/`;
        const { resources, menuResourceCount, gameResourceCount } =
            this.collectImageResources(gameBaseUrl);
        ...
        const menuTracked = (tag: string) => tag === this.MENU_TAG || tag === this.FONT_TAG;
        const gameTracked = (tag: string) => tag === this.GAME_TAG;
        const finalizeMenu = () => {
        ...
        const finalizeGame = () => {
            if (gameLoaded + gameFailed === gameResourceCount) {
                this.gameImagesLoadComplete = true;
                if (gameFailed > 0) {
                    window.console?.warn?.(
                        `Game image loading completed with failures - Images: ${gameFailed}`
                    );
                }
                this.checkGameLoadComplete();
            }
        };
    ...
    runWhenGameReady(onComplete: (() => void) | null): void {
        if (!onComplete) {
            return;
        }
        if (this.gameImagesLoadComplete) {
            setTimeout(onComplete, 0);
            return;
        }
        this.gameCompleteCallbacks.push(onComplete);
    }
