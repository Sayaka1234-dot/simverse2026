...
def get_qwen_open_results_dir(model_name: str, result_namespace: str | None = None) -> Path:
...
def build_qwen_open_result_path(
    ...
    *,
    ...
    result_root = get_qwen_open_results_dir(model_name, result_namespace=result_namespace)
    try:
...
def build_qwen_open_result_path_for_sample(
    sample_id: str,
    *,
    model_name: str,
    result_namespace: str | None = None,
) -> Path:
    return (get_qwen_open_results_dir(model_name, result_namespace=result_namespace) / f"{sample_id}.json").resolve()


def save_qwen_open_result_json(
    task_json_path: str | Path,
    ...
    *,
    ...
    result_path = build_qwen_open_result_path(task_json_path, model_name=model_name, result_namespace=result_namespace)
    result_path.parent.mkdir(parents=True, exist_ok=True)
    result_path.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
    return result_path


def save_qwen_open_result_json_for_sample(
    sample_id: str,
    payload: dict[str, Any],
    *,
    model_name: str,
    ...
    result_path = build_qwen_open_result_path_for_sample(sample_id, model_name=model_name, result_namespace=result_namespace)
    result_path.parent.mkdir(parents=True, exist_ok=True)
    result_path.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
    return result_path
...
def request_qwen_open_answer(task: GoalRollTask, model_name: str) -> tuple[str, ModelAnswer, str]:
    if not QWEN_OPEN_API_KEY or QWEN_OPEN_API_KEY == "YOUR_QWEN_OPEN_API_KEY":
        raise RuntimeError(
            "QWEN_OPEN_API_KEY is empty. Please set QWEN_OPEN_API_KEY or edit cube2/eval-thinking/eval_qwen_open_local.py."
        )
...
def evaluate_single_task_json(
    ...
    *,
    ...
    result_path = save_qwen_open_result_json(
        task_path,
        payload,
        model_name=model_name,
...
def evaluate_single_manifest_task(
    ...
    *,
    ...
    result_path = save_qwen_open_result_json_for_sample(
        task.sample_id,
        payload,
        model_name=model_name,
        result_namespace=result_namespace,
    ...
    payload["result_json_path"] = str(result_path)
    save_qwen_open_result_json_for_sample(
        task.sample_id,
        payload,
        model_name=model_name,
        result_namespace=result_namespace,
...
def evaluate_batch_task_jsons(
    ...
    *,
    ...
    for index, task_path in enumerate(task_files, 1):
        result_path = build_qwen_open_result_path(task_path, model_name=model_name, result_namespace=result_namespace)
...
def evaluate_batch_manifest(
    ...
    *,
    ...
    for index, task in enumerate(tasks, 1):
        result_path = build_qwen_open_result_path_for_sample(task.sample_id, model_name=model_name, result_namespace=result_namespace)
        if skip_existing and result_path.exists():
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate cube2 goal-roll tasks with Qwen Open.")
    parser.add_argument(
        "task_target",
        nargs="?",
