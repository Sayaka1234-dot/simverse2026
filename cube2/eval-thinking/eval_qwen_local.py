...
def get_qwen_results_dir(model_name: str, result_namespace: str | None = None) -> Path:
    model_dir = CURRENT_DIR / "results" / sanitize_namespace(model_name)
    namespace = sanitize_namespace(result_namespace or model_name)
    return model_dir / namespace


def build_qwen_result_path(
    ...
    *,
    ...
    result_root = get_qwen_results_dir(model_name, result_namespace=result_namespace)
    try:
...
def build_qwen_result_path_for_sample(
    sample_id: str,
    *,
    model_name: str,
    result_namespace: str | None = None,
) -> Path:
    return (get_qwen_results_dir(model_name, result_namespace=result_namespace) / f"{sample_id}.json").resolve()


def save_qwen_result_json(
    task_json_path: str | Path,
    ...
    *,
    ...
    result_path = build_qwen_result_path(task_json_path, model_name=model_name, result_namespace=result_namespace)
    result_path.parent.mkdir(parents=True, exist_ok=True)
    result_path.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
    return result_path


def save_qwen_result_json_for_sample(
    sample_id: str,
    payload: dict[str, Any],
    *,
    model_name: str,
    ...
    result_path = build_qwen_result_path_for_sample(sample_id, model_name=model_name, result_namespace=result_namespace)
    result_path.parent.mkdir(parents=True, exist_ok=True)
    result_path.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
    return result_path
...
def build_qwen_client() -> tuple[Any, httpx.Client]:
    ...
    timeout = httpx.Timeout(
        ...
        connect=min(QWEN_TIMEOUT_SECONDS, 30.0),
    )
...
def request_qwen_answer(task: GoalRollTask, model_name: str) -> tuple[str, ModelAnswer, str]:
    if not QWEN_API_KEY:
        raise RuntimeError("QWEN_API_KEY is empty. Please edit cube2/eval-thinking/eval_qwen_local.py.")
    ...
    try:
        ...
        raw_text, reasoning_output = extract_message_output_parts(completion.choices[0].message)
...
def evaluate_task(
    ...
    *,
    ...
    raw_model_output, answer, model_reasoning_output = request_qwen_answer(task, model_name)
...
def evaluate_single_task_json(
    ...
    *,
    ...
    result_path = save_qwen_result_json(
        task_path,
        payload,
        model_name=model_name,
...
def evaluate_single_manifest_task(
    ...
    *,
    ...
    result_path = save_qwen_result_json_for_sample(
        task.sample_id,
        payload,
        model_name=model_name,
        result_namespace=result_namespace,
    ...
    payload["result_json_path"] = str(result_path)
    save_qwen_result_json_for_sample(
        task.sample_id,
        payload,
        model_name=model_name,
        result_namespace=result_namespace,
...
def evaluate_batch_task_jsons(
    ...
    *,
    ...
    for index, task_path in enumerate(task_files, 1):
        result_path = build_qwen_result_path(task_path, model_name=model_name, result_namespace=result_namespace)
...
def evaluate_batch_manifest(
    ...
    *,
    ...
    for index, task in enumerate(tasks, 1):
        result_path = build_qwen_result_path_for_sample(task.sample_id, model_name=model_name, result_namespace=result_namespace)
        if skip_existing and result_path.exists():
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate cube2 goal-roll tasks with Qwen.")
    parser.add_argument(
        "task_target",
        nargs="?",
