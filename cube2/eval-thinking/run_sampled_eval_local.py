    ...
    *,
    ...
    for index, task_path in enumerate(sampled_files, 1):
        ...
        if skip_existing and result_path.exists():
            print(f"\n[{index}/{len(sampled_files)}] [skip] {task_path.name}")
            try:
        ...
        print(f"\n[{index}/{len(sampled_files)}] evaluating {task_path.name}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Sample cube2 tasks by tier and evaluate them.")
    parser.add_argument("--dataset-root", type=Path, default=TASK_JSON_ROOT, help="Task JSON directory.")
    parser.add_argument("--total", type=int, default=DEFAULT_TOTAL, help="Total sampled task count.")
