...
DEFAULT_DEEPSEEK_MODEL_NAME = os.getenv("DEEPSEEK_MODEL_NAME", "deepseek-v4-pro")
DEFAULT_LEVELS_DIR = PROJECT_ROOT / "levels"
DEFAULT_RESULTS_DIR = EVAL_ROOT / "results" / DEFAULT_DEEPSEEK_MODEL_NAME
...
DEFAULT_DEEPSEEK_TIMEOUT_SECONDS = float(os.getenv("DEEPSEEK_TIMEOUT_SECONDS", "900"))
DEFAULT_DEEPSEEK_TRUST_ENV = os.getenv(
    "DEEPSEEK_TRUST_ENV",
...
DEFAULT_DEEPSEEK_REASONING_EFFORT = os.getenv("DEEPSEEK_REASONING_EFFORT", "high")
...
def build_argument_parser(default_level: str = "level001") -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run single-level or batch evaluation for Text-VOI with DeepSeek models."
    )
    parser.add_argument(
        "--level",
    ...
    parser.add_argument("--model", default=DEFAULT_DEEPSEEK_MODEL_NAME)
    parser.add_argument("--base-url", default=DEFAULT_DEEPSEEK_BASE_URL)
    parser.add_argument("--api-key", default=DEFAULT_DEEPSEEK_API_KEY)
    parser.add_argument(
        "--timeout-seconds",
        type=float,
    ...
    parser.add_argument(
        ...
        default=DEFAULT_DEEPSEEK_TRUST_ENV,
    ...
    parser.add_argument(
        ...
        help="Reasoning effort passed to the DeepSeek API. Default: high.",
    )
    parser.add_argument(
        "--show-reference",
        action="store_true",
...
def print_request_config(args: argparse.Namespace) -> None:
    print("=== DeepSeek Request Config ===")
    print("provider          : deepseek-openai-compatible")
...
def request_operation_code_with_retry(
    ...
    for attempt_index, attempt_model_name in enumerate(attempt_plan, start=1):
        ...
        print(
            "Thinking payload: "
            f"requested_mode={completion.get('thinking_request_mode', 'enabled')} "
            f"reasoning_effort={completion.get('reasoning_effort', args.reasoning_effort)}"
        )
    ...
    return {
        ...
        "thinking_request_mode": last_completion.get("thinking_request_mode", "enabled"),
        "reasoning_effort": last_completion.get("reasoning_effort", args.reasoning_effort),
...
def build_result_payload(
    ...
    return {
        ...
        "completion": {
            ...
            "reasoning_effort": completion_data.get("reasoning_effort"),
...
def evaluate_one_level(
    ...
    print(f"=== Evaluating {level_path.stem} with DeepSeek ===")
...
def main(default_level: str = "level001") -> int:
    ...
    if not args.api_key:
        raise SystemExit("未设置 DEEPSEEK_API_KEY，无法运行 DeepSeek 评测。")
