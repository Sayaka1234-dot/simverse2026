...
DEFAULT_OPENAI_MODEL_NAME = os.getenv("OPENAI_MODEL_NAME", "gemini-3.1-pro-preview")
DEFAULT_LEVELS_DIR = PROJECT_ROOT / "levels"
DEFAULT_RESULTS_DIR = EVAL_ROOT / "results" / DEFAULT_OPENAI_MODEL_NAME
DEFAULT_JSON_PATH = DEFAULT_LEVELS_DIR / "level001.json"
DEFAULT_OPENAI_BASE_URL = os.getenv("OPENAI_BASE_URL", "https://xiaoai.plus/v1")
DEFAULT_OPENAI_API_KEY = os.getenv("OPENAI_API_KEY", "")
DEFAULT_OPENAI_TIMEOUT_SECONDS = float(os.getenv("OPENAI_TIMEOUT_SECONDS", "900"))
DEFAULT_OPENAI_TRUST_ENV = os.getenv("OPENAI_TRUST_ENV", "false").strip().lower() in {
    "1",
    "true",
    "yes",
    "on",
...
def resolve_default_max_retries(model_name: str) -> int:
    return 2 if looks_like_gemini_model(model_name) else 1


def build_argument_parser(default_level: str = "level001") -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run single-level or batch evaluation for Text-VOI."
    )
    parser.add_argument(
        "--level",
    ...
    parser.add_argument("--model", default=DEFAULT_OPENAI_MODEL_NAME)
    ...
    parser.add_argument(
        ...
        default=DEFAULT_OPENAI_TRUST_ENV,
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Automatic retry count for suspected truncated outputs. Default: 2 for Gemini, 1 for others.",
    )
    parser.add_argument(
        "--fallback-model",
        default=None,
        help="Optional non-preview fallback model used after truncation retries are exhausted.",
    )
    parser.add_argument(
        "--show-reference",
        action="store_true",
...
def evaluate_one_level(
    ...
    print(f"=== Evaluating {level_path.stem} ===")
...
def main(default_level: str = "level001") -> int:
    ...
    if not args.api_key:
        raise SystemExit("未设置 API key，无法调用模型。")
