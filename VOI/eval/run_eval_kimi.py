...
DEFAULT_KIMI_MODEL_NAME = os.getenv("KIMI_MODEL_NAME", "kimi-k2.6")
DEFAULT_LEVELS_DIR = PROJECT_ROOT / "levels"
DEFAULT_RESULTS_DIR = EVAL_ROOT / "results" / DEFAULT_KIMI_MODEL_NAME
DEFAULT_JSON_PATH = DEFAULT_LEVELS_DIR / "level001.json"
DEFAULT_KIMI_BASE_URL = os.getenv("KIMI_BASE_URL", "https://api.moonshot.cn/v1")
DEFAULT_KIMI_API_KEY = os.getenv("MOONSHOT_API_KEY", "")
DEFAULT_KIMI_TIMEOUT_SECONDS = float(os.getenv("KIMI_TIMEOUT_SECONDS", "900"))
DEFAULT_KIMI_TRUST_ENV = os.getenv(
    "KIMI_TRUST_ENV",
...
def build_argument_parser(default_level: str = "level001") -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run single-level or batch evaluation for Text-VOI with Kimi models."
    )
    parser.add_argument(
        "--level",
    ...
    parser.add_argument("--model", default=DEFAULT_KIMI_MODEL_NAME)
    parser.add_argument("--base-url", default=DEFAULT_KIMI_BASE_URL)
    parser.add_argument("--api-key", default=DEFAULT_KIMI_API_KEY)
    parser.add_argument(
        "--timeout-seconds",
        type=float,
    ...
    parser.add_argument(
        ...
        default=DEFAULT_KIMI_TRUST_ENV,
...
def print_request_config(args: argparse.Namespace) -> None:
    print("=== Kimi Request Config ===")
    print("provider        : moonshot-openai-compatible")
...
def evaluate_one_level(
    ...
    print(f"=== Evaluating {level_path.stem} with Kimi ===")
...
def main(default_level: str = "level001") -> int:
    ...
    if not args.api_key:
        raise SystemExit("未设置 MOONSHOT_API_KEY，无法运行 Kimi 评测。")
