...
DEFAULT_GLM_MODEL_NAME = os.getenv("GLM_MODEL_NAME", "glm-5v-turbo")
DEFAULT_GLM_BASE_URL = os.getenv("GLM_BASE_URL", "")
DEFAULT_GLM_API_KEY = os.getenv("GLM_API_KEY", "1042338736b9443d980979cdfe935688.Zi0RmJGJCvkKP9ZW")
DEFAULT_GLM_TIMEOUT_SECONDS = float(os.getenv("GLM_TIMEOUT_SECONDS", "900"))
DEFAULT_GLM_THINKING_ENABLED = os.getenv("GLM_THINKING_ENABLED", "true").strip().lower() in {
    "1",
    "true",
    "yes",
    "on",
...
def build_argument_parser(default_level: str = "level001") -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run single-level or batch evaluation for Text-VOI with GLM models."
    )
    parser.add_argument(
        "--level",
    ...
    parser.add_argument("--model", default=DEFAULT_GLM_MODEL_NAME)
    parser.add_argument("--base-url", default=DEFAULT_GLM_BASE_URL)
    parser.add_argument("--api-key", default=DEFAULT_GLM_API_KEY)
    parser.add_argument(
        "--timeout-seconds",
        type=float,
        default=DEFAULT_GLM_TIMEOUT_SECONDS,
    )
    parser.add_argument(
        "--thinking-enabled",
        action=argparse.BooleanOptionalAction,
        default=DEFAULT_GLM_THINKING_ENABLED,
        help="Whether to enable GLM thinking mode.",
    )
...
def build_glm_client(api_key: str, base_url: str, timeout_seconds: float):
    ...
    try:
        return ZhipuAiClient(**client_kwargs)
    except TypeError:
        # Some SDK versions may not expose base_url / timeout in the constructor.
        return ZhipuAiClient(api_key=api_key)


def print_request_config(args: argparse.Namespace, results_dir: Path) -> None:
    print("=== GLM Request Config ===")
    print("provider        : zhipu-zai")
    print(f"base_url        : {args.base_url or '<sdk-default>'}")
    print(f"model           : {args.model}")
    print(f"timeout_seconds : {args.timeout_seconds}")
    print(f"thinking_enabled: {args.thinking_enabled}")
    print(
        "thinking_mode   : "
        f"{'provider-default (reasoning allowed)' if args.thinking_enabled else 'disabled (explicit payload requested)'}"
    )
...
def request_operation_completion(
    ...
    thinking_request_mode = "provider-default" if thinking_enabled else "disabled"
    ...
    thinking_parameter_supported = None if thinking_enabled else True
    thinking_parameter_removed_for_fallback = None if thinking_enabled else False
    ...
    try:
        ...
        thinking_parameter_supported = False
        thinking_parameter_removed_for_fallback = True
    ...
    return {
        ...
        "thinking_parameter_supported": thinking_parameter_supported,
        "thinking_parameter_removed_for_fallback": thinking_parameter_removed_for_fallback,
...
def request_operation_code_with_retry(
    ...
    for attempt_index in range(1, total_attempts + 1):
        ...
        thinking_request_mode = completion.get("thinking_request_mode") or (
            "provider-default" if args.thinking_enabled else "disabled"
        )
        thinking_parameter_supported = completion.get("thinking_parameter_supported")
        thinking_parameter_removed_for_fallback = completion.get(
            "thinking_parameter_removed_for_fallback"
        )
        print(
            "Thinking payload: "
            f"requested_mode={thinking_request_mode} "
            f"supported={thinking_parameter_supported if thinking_parameter_supported is not None else 'unknown'} "
            "fallback_removed="
            f"{thinking_parameter_removed_for_fallback if thinking_parameter_removed_for_fallback is not None else 'unknown'}"
        )
    ...
    return last_completion if last_completion is not None else {
        ...
        "thinking_request_mode": "provider-default" if args.thinking_enabled else "disabled",
        "thinking_parameter_supported": False,
        "thinking_parameter_removed_for_fallback": False,
...
def build_result_payload(
    ...
    return {
        ...
        "completion": {
            ...
            "thinking_parameter_supported": completion_data.get("thinking_parameter_supported"),
            "thinking_parameter_removed_for_fallback": completion_data.get(
                "thinking_parameter_removed_for_fallback"
            ),
...
def evaluate_one_level(
    ...
    print(f"=== Evaluating {level_path.stem} with GLM ===")
...
def main(default_level: str = "level001") -> int:
    ...
    if not args.api_key or args.api_key == "your-api-key":
        raise SystemExit("未设置 GLM_API_KEY，无法调用 GLM 模型。")
