...
def request_qwen_answer(task: PuzzleTask, model_name: str) -> tuple[str, ModelAnswer, str]:
    if not QWEN_API_KEY:
        raise RuntimeError("QWEN_API_KEY is empty. Please edit eval-thinking/eval_qwen_local.py.")
...
def evaluate_single_task_json(
    ...
    *,
    ...
    try:
        ...
        if isinstance(exc, ModelOutputParseError) and exc.raw_text:
            print(f"Raw Output:\n{exc.raw_text}")
        return None
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate cube reconstruction tasks with Qwen.")
    parser.add_argument(
        "target_path",
        type=Path,
        nargs="?",
...
def main() -> None:
    args = parse_args()
    result_namespace = build_result_namespace(QWEN_MODEL_NAME, args.result_namespace)
