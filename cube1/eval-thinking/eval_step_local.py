...
def request_step_answer(task: PuzzleTask, model_name: str) -> tuple[str, ModelAnswer, str]:
    if not STEP_API_KEY or STEP_API_KEY == "YOUR_STEPFUN_APIKEY":
        raise RuntimeError("STEP_API_KEY is empty. Please set STEP_API_KEY or edit eval-thinking/eval_step_local.py.")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate cube reconstruction tasks with Step.")
    parser.add_argument(
        "target_path",
        type=Path,
        nargs="?",
...
def main() -> None:
    args = parse_args()
    result_namespace = build_result_namespace(STEP_MODEL_NAME, args.result_namespace)
