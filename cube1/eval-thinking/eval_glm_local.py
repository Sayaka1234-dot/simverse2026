...
GLM_MAX_TOKENS = 20000
GLM_MIN_REQUEST_INTERVAL_SECONDS = float(os.getenv("GLM_MIN_REQUEST_INTERVAL_SECONDS", "6.5"))
...
def throttle_glm_request_rate(
    *,
    ...
    delay_seconds = compute_glm_request_delay_seconds(
        ...
        min_interval_seconds=GLM_MIN_REQUEST_INTERVAL_SECONDS,
    )
    if delay_seconds > 0:
...
def build_glm_client() -> Any:
    ...
    return ZhipuAiClient(api_key=GLM_API_KEY, base_url=GLM_BASE_URL)
...
def request_glm_answer(task: PuzzleTask, model_name: str) -> tuple[str, ModelAnswer, str]:
    if not GLM_API_KEY or GLM_API_KEY == "YOUR_GLM_API_KEY":
        raise RuntimeError("GLM_API_KEY is empty. Please set GLM_API_KEY or edit eval-thinking/eval_glm_local.py.")

    messages = build_multimodal_messages_for_eval(task)

    client = build_glm_client()
    throttle_glm_request_rate()
    ...
    if reasoning_output:
        print(f"\n[Model Reasoning Output] {task.sample_id} | {model_name}")
        print(reasoning_output)
        print("[End Model Reasoning Output]\n")
    ...
    *,
    ...
    try:
        ...
        if should_skip_result_write(exc):
            print(
                f"Skip {task.sample_id}: transient API failure or incomplete model output, "
                f"no result file will be written.\nReason: {describe_skip_reason(exc)}"
            )
            return None
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate cube reconstruction tasks with GLM.")
    parser.add_argument(
        "target_path",
        type=Path,
        nargs="?",
...
def main() -> None:
    args = parse_args()
    result_namespace = build_result_namespace(GLM_MODEL_NAME, args.result_namespace)
