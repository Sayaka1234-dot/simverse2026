 ...
 * Supports rotation plus optional horizontal / vertical mirroring.
 */

const BASE_TEXT_FONT = '"Outfit", "Trebuchet MS", sans-serif';
const TEXT_COLORS = [
...
function normalizePatternTransform(transform) {
    return {
        flipHorizontal: Boolean(transform?.flipHorizontal ?? transform?.flipX),
        flipVertical: Boolean(transform?.flipVertical ?? transform?.flipY)
    };
}
...
function resolvePatternFace(patternIdOrFace, rotation, transformOptions) {
    if (patternIdOrFace && typeof patternIdOrFace === 'object' && typeof patternIdOrFace.patternId === 'string') {
        return {
            ...
            ...normalizePatternTransform(patternIdOrFace)
        };
    }
    ...
    return {
        ...
        ...normalizePatternTransform(transformOptions)
    };
}

/**
 * Draw a pattern onto a canvas context with rotation and optional mirroring.
...
function drawPattern(ctx, patternIdOrFace, rotation, x, y, size, bgColor, transformOptions) {
    ...
    ctx.scale(face.flipHorizontal ? -1 : 1, face.flipVertical ? -1 : 1);
