...
DEFAULT_MODEL_NAME = os.getenv("EVAL_MODEL_NAME", "gemini-3.1-pro-preview")
DEFAULT_BASE_URL = os.getenv("OPENAI_BASE_URL", "https://xiaoai.plus/v1")
DEFAULT_API_KEY = os.getenv("OPENAI_API_KEY", os.getenv("GEMINI_API_KEY", ""))
...
def evaluate_task(task: Any, *, task_source: str, model_name: str) -> dict[str, Any]:
    if not DEFAULT_API_KEY:
        raise RuntimeError("OPENAI_API_KEY is required for eval-thinking/eval_gemini_local.py")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate mechanical lamp tasks in thinking mode with Gemini (via OpenAI compatible proxy).")
